...
def frequencies(string):
    ...
    occurences = list(map(lambda index: string.count(letter(index)) if letter(index) != " " else 0, range(26)))
    return list(map(lambda char: char / sum(occurences) * 100, occurences))
...
"""
To compare the observed frequencies in the encoded string with the expected average frequencies, we can use the chi-square 
...
def chisqr(observed, expected):
    """
     ...
     index i.
    """
    #return sum(map(lambda i: (observed[i]-expected[i])**2/expected[i],range(26)))
    return sum(map(lambda x,y: (x-y)**2/y, observed, expected))
...
def crack(encoded_string, reference_table):
    """
     ...
     value.
    """
    freq = frequencies(encoded_string)
    chilist = [ chisqr(rotate(freq,n),reference_table) for n in range(26)]
    shft = chilist.index(min(chilist))
    return encode(encoded_string,-shft)
