...
def is_last_column_empty():
    ...
    return not any(map(lambda x: x is not None,
                       column(be_indicators, len(be_indicators[0]) - 1)))
...
def select_rows(attribute, instances, matrix):
    ...
    return list(filter(lambda x: x[matrix[0].index(attribute)] in instances,matrix))
...
def processed_belgium_development_indicators_matrix(matrix):
    ...
    headed_row_data = [matrix[0]] + filtered_row_data
    annual_data = list(map(lambda x : x[year_1960:year_2013],headed_row_data))
    features_data = transpose(annual_data)
    filtered_features_data = list(filter(lambda x: any(map(lambda x: x is not None, x[1:])),features_data))
    return [['Year'] + features_of_interest] + filtered_features_data
...
for n in range(len(final_matrix)):
